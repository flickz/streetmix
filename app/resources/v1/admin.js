...
exports.post = function (req, res) {
  ...
  if (!body.fullName || !body.email || !body.birthDate) {
    res.status(400).send('Please provide all specified fields')
    return
  }
  if (!(isEmail(body.email) && isISO8601(body.birthDate))) {
    res.status(400).send('Please ensure email and date of birth are valid')
    return
  }
